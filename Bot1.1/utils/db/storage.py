...
class DatabaseManager(object):
    ...
    def create_tables(self):
        ...
        self.query(
            "CREATE TABLE IF NOT EXISTS orders (cid int, usr_name text, usr_address text, usr_phone text, "
            "products text) "
        )
        self.query("CREATE TABLE IF NOT EXISTS cart (cid int, idx text, quantity int)")
        self.query(
            "CREATE TABLE IF NOT EXISTS categories (idx text, parentID int, title text)"
        )
...
orders: cid int, usr_name text, usr_address text, usr_phone text, products text

cart: cid int, idx text, quantity int ==> product_idx

categories: idx text, parentID int, title text
