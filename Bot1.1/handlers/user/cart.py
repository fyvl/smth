...
async def process_check_cart_all_right(message: Message, state: FSMContext):
    ...
    await CheckoutState.next()
    await message.answer("Укажите номер телефона.", reply_markup=back_markup())


@dp.message_handler(IsUser(), text=back_message, state=CheckoutState.number)
async def process_name_back(message: Message, state: FSMContext):
    await CheckoutState.check_cart.set()
    await checkout(message, state)
...
async def process_name(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        if "address" in data.keys():
            ...
            await CheckoutState.next()
            await message.answer(
                "Укажите адрес доставки.", reply_markup=back_markup()
            )
    ...
    if enough_money:
        ...
        async with state.proxy() as data:
            ...
            db.query(
                ...
                (cid, data["name"], data["address"],  data["number"], " ".join(products)),
            )

            db.query("DELETE FROM cart WHERE cid=?", (cid,))

            await message.answer(
                "Ваш заказ оформлен, с Вами свяжется оператор для подтверждения заказа! 🚀\nИмя: <b>"
                + data["name"]
                + "</b>\nАдрес: <b>"
                + data["address"]
                + "</b>\nНомер телефона: <b>"
            ...
            await bot.send_message(chat_id=841439263, text="Заказ от: " + data["name"] + "\nАдрес: " + data["address"] + "\nНомер телефона: " + data["number"]
